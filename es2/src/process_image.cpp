...
Image rgb_to_grayscale(const Image &im)
{
  ...
  for (int i = 0; i < im.w; i++)
  {
    ...
    {
      float gray_val = 0.299 * im.clamped_pixel(i, j, 0) +
                       0.587 * im.clamped_pixel(i, j, 1) + 0.114 * im.clamped_pixel(i, j, 2);
      gray(i, j, 0) = gray_val;
    }
  }
  return gray;
...
void clamp_image(Image &im)
{
  for (int k = 0; k < im.c; k++)
  {
    for (int i = 0; i < im.w; i++)
    {
      for (int j = 0; j < im.h; j++)
      {
        float val = im.clamped_pixel(i, j, k);
        if (val < 0)
        {
          val = 0;
        }
...
void rgb_to_hsv(Image &im)
{
  ...
  for (int i = 0; i < im.w; i++)
  {
    for (int j = 0; j < im.h; j++)
    {
      float r = im.clamped_pixel(i, j, 0);
      float g = im.clamped_pixel(i, j, 1);
      float b = im.clamped_pixel(i, j, 2);
...
void hsv_to_rgb(Image &im)
{
  ...
  for (int i = 0; i < im.w; i++)
  {
    for (int j = 0; j < im.h; j++)
    {
      float h = im.clamped_pixel(i, j, 0);
      float s = im.clamped_pixel(i, j, 1);
      float v = im.clamped_pixel(i, j, 2);
