...
Image make_1d_gaussian(float sigma)
 ...
 int w = ceil (sigma * 6);
  ...
  for(int i = 0; i < lin.w; i++){
    float x = i - lin.w/2;
    lin.data[i] = (1.0/(2*M_PI*sigma*sigma)) *exp(-x*x/(2*sigma*sigma));
  }
...
Image smooth_image(const Image& im, float sigma)
  ...
  ret = convolve_image(ret, gauss_1d, true);
  swap(gauss_1d.w, gauss_1d.h);
  ret = convolve_image(ret, gauss_1d, true);
...
Image structure_matrix(const Image& im2, float sigma)
  ...
  Image Ix = convolve_image(im, make_gx_filter(), false);
  Image Iy = convolve_image(im, make_gy_filter(), false);
  Image gauss = make_gaussian_filter(sigma);

  for(int x = 0; x < S.w; x++){
    for(int y = 0; y < S.h; y++){
      S(x,y,0) = Ix(x,y)*Ix(x,y);
      S(x,y,1) = Iy(x,y)*Iy(x,y);
      S(x,y,2) = Ix(x,y)*Iy(x,y);
    } 
  }
...
Image cornerness_response(const Image& S, int method)
  {
  Image R(S.w, S.h);
  //assert(method == 2 && "Non hai scelto l'unico metodo giusto, ha!");
      ...
      case(1):
        for(int x = 0; x < S.w; x++){
          for(int y = 0; y < S.h; y++){
            ...
            float det = (a*b) - (c*c);
            float trace = (a+b);
            R(x, y) = det - pow(trace,2);
          }
        }
        break;
      case(2):
        printf("Non implementata :3\n");
        for(int x = 0; x < S.w; x++){
          for(int y = 0; y < S.h; y++){
            float a = S(x,y, 0);
            float b = S(x, y, 1);
            ...
            float radix = sqrt(pow(a+b,2) + 4*(pow(c,2)-(a*b)));
            float s1 = (a+b+radix) / 2;
            float s2 = (a+b-radix) / 2;
  ...
  for(int c = 0; c < im.c; c++){
    for(int x = 0; x < im.w; x++){
      for(int y = 0; y < im.h; y++){
        float val = im.clamped_pixel(x, y, c);
        bool skip = false;
        for(int i = -w; i<= w & !skip; i++){
          for(int j = -w; j <= w & !skip; j++){
            if(im.clamped_pixel(x+i, y+j, c) > val){
              skip = true;  
            }
          }
        }
