...
Image testing_image = Image(2,2,3);
...
void test_get_pixel()
{
  ...
  for (int z = 0; z < 3; z++)
  {
    for (int y = 0; y < 2; y++)
    {
      for (int x = 0; x < 4; x++)
      {
        //printf("Testing color at %d %d %d\tExpected = %d Actual = %d\n", x, y, z, expected_res[i_test],im.clamped_pixel(x, y, z));
        TEST(within_eps(expected_res[i_test++], im.clamped_pixel(x, y, z)));
      }
    }
  }
...
void test_grayscale()
{
  ...
  gray.save_image("output/grayscaleImage.png");
...
void test_shift()
{
  ...
  shift_image(im, 1, .1);
  TEST(same_image(im, c));
  TEST(im.data[0] == c.data[0]);
  TEST(im.data[im.w * im.h + 13] == c.data[im.w * im.h + 13]);
  TEST(im.data[2 * im.w * im.h + 72] == c.data[2 * im.w * im.h + 72]);
  TEST(im.data[im.w * im.h + 47] == c.data[im.w * im.h + 47]);
}
